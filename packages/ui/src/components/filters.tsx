...
import { ButtonGroup, ButtonGroupText } from "@starter/ui/components/button-group";
import {
  DropdownMenu,
...
function FilterRemoveButton({ icon = <XIcon />, ...props }: FilterRemoveButtonProps) {
  const context = useFilterContext();
...
type FilterValues<T = unknown> = Array<T | string>;
...
export interface FilterFieldConfig<T = unknown> {
  ...
  customValueRenderer?: (values: FilterValues<T>, options: FilterOption<T>[]) => React.ReactNode;
  placeholder?: string;
  searchable?: boolean;
  ...
  onValueChange?: (values: FilterValues<T>) => void;
}
...
interface FilterSubmenuContentProps<T = unknown> {
  field: FilterFieldConfig<T>;
  currentValues: FilterValues<T>;
  isMultiSelect: boolean;
  onToggle: (value: T | string, isSelected: boolean) => void;
  i18n: FilterI18nConfig;
  isActive?: boolean;
  onActive?: () => void;
  onBack?: () => void;
  ...
  const addFilter = useCallback(
    (fieldKey: string) => {
      ...
      if (field && field.key) {
        ...
        const defaultValues: FilterValues<T> = field.type === "text" ? [""] : [];
