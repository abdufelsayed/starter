...
function isStepperItemElement(child: unknown): child is ReactElement<StepperItemProps> {
...
function Stepper({
  ...
  const unregisterTrigger = useCallback((node: HTMLButtonElement | null) => {
    ...
    setTriggerNodes((prev) => prev.filter((registeredNode) => registeredNode !== node));
  }, []);
  ...
  const stepsCount = useMemo(
    () => Children.toArray(children).filter(isStepperItemElement).length,
    [children],
  );
...
function StepperTrigger({
  ...
  const setButtonRef = useCallback(
    (node: HTMLButtonElement | null) => {
      if (btnRef.current && btnRef.current !== node) {
        unregisterTrigger(btnRef.current);
      }
