...
import { Calendar, CalendarDayButton } from "@starter/ui/components/calendar";
...
const DATE_SELECTOR_FILTER_TYPES = ["is", "before", "after", "between"] as const;
const DATE_SELECTOR_PERIOD_TYPES = ["day", "month", "quarter", "half-year", "year"] as const;

function isDateSelectorFilterType(value: string): value is DateSelectorFilterType {
  return DATE_SELECTOR_FILTER_TYPES.some((type) => type === value);
}

function isDateSelectorPeriodType(value: string): value is DateSelectorPeriodType {
  return DATE_SELECTOR_PERIOD_TYPES.some((type) => type === value);
}
  ...
  return (
    <Tabs
      ...
      onValueChange={(newValue) => {
        if (!isDisabled && isDateSelectorFilterType(newValue)) {
          onChange(newValue);
        }
      }}
      className={className}
  ...
  return (
    <div className={cn("flex flex-wrap items-center justify-between gap-3", className)}>
      <Tabs
        value={value}
        onValueChange={(newValue) => {
          if (isDateSelectorPeriodType(newValue)) {
            onChange(newValue);
          }
        }}
      >
  ...
  const selected: Date | DateRange | undefined = isRange
    ? selectedDate && selectedEndDate
      ...
      : selectedDate
        ...
        : undefined
    : selectedDate;
  const selectedRange = selected && !(selected instanceof Date) ? selected : undefined;
  const selectedSingle = selected instanceof Date ? selected : undefined;
